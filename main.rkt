...
(define substitution? (and/c (listof association?) no-duplicate-cars?)) ; and no cycles
...
(define (walk val sub)
  (let ([association (and (var? val) (assv val sub))])
    (if (pair? association) ; val is a var and lookup succeeded
        (walk (cdr association) s) ; recursion is necessary in case val is associated to another var
        val)))
...
(define substitution-stream? (stream-of element/c))
...
(define ((reify val) sub)
  (let ([val (walk* val sub)]
        [rsub (reify-sub val)])
    ; this reifies tha variables according to the reified name substitution
    (walk* val rsub)))
